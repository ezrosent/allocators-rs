...
use super::utils::{self, MMAP, Lazy, LazyInitializable};
...
impl<M: MemorySource> PageSource<M> {
    fn new(
        ...
        Some(PageSource {
            ...
            pages: SlagPipe::new_size_cleanup(pipe_size, PageCleanup::new(page_size, page_size)),
            source: m,
        })
    }
    ...
    unsafe fn free(&mut self, p: *mut u8, old_size: usize) {
        ...
        if self.pages.size_guess() >= self.target_size as isize {
            ...
            (&*MMAP).dealloc(p, layout);
        } else {
            if old_size >= self.cutoff_bytes {
                let layout = Layout::from_size_align(self.source.page_size() - self.cutoff_bytes, page_size).unwrap();
                (&*MMAP).uncommit(p.offset(self.cutoff_bytes as isize), layout);
            }
...
impl<M: MemorySource> PageFrontend<M> {
    fn new(
        ...
        Some(PageFrontend {
            parent: parent,
            pages: SlagPipe::new_size_cleanup(pipe_size, PageCleanup::new(size, size)),
            local_size: size,
...
impl ElfMallocBuilder {
    pub fn page_size(&mut self, page_size: usize) -> &mut ElfMallocBuilder {
        self.page_size = cmp::max(utils::page_size(), page_size);
        self
    }
    ...
    pub fn build<M: MemorySource>(&self) -> Option<ElfMalloc<M>> {
        ...
        let layout = Layout::new::<Metadata>().repeat(n_small_classes).unwrap().0;
        let mut meta_pointers = unsafe { (&*MMAP).alloc(layout).ok()? as *mut Metadata };
        let small_classes = Multiples::init(MULTIPLE, n_small_classes, |size: usize| {
            ...
            let params = (
                ...
                pa.clone(),
                RevocablePipe::new_size_cleanup(self.small_pipe_size, PageCleanup::new(self.page_size, self.page_size)),
            );
