...
use super::utils::{self, Lazy, TypedArray, likely};
...
unsafe fn elfmalloc_get_layout(page_size: usize, item: *mut u8) -> (usize, usize) {
    match get_type(item) {
        ...
        AllocType::Large => (large_alloc::get_size(item), utils::page_size()),
    }
}
...
impl<M: MemorySource, D: DirtyFn, AM: AllocMap<ObjectAlloc<PageAlloc<M, D>>, Key = usize>>
    ...
    fn new_internal(
        ...
        use utils::MMAP;
        let layout = Layout::new::<Metadata>().repeat(n_classes).unwrap().0;
        let mut meta_pointer = unsafe { (&*MMAP).alloc(layout).map(|ptr| ptr as *mut Metadata).ok()? };
        ...
        let am = AM::init(start_from, n_classes, |size: usize| {
            ...
            let clean = PageCleanup::new(pa.page_size(), pa.page_size());
    ...
    pub unsafe fn free(item: *mut u8) {
        ...
        #[cfg(debug_assertions)]
        {
            ...
            alloc_debug_assert_eq!(
                base_ptr as usize % utils::page_size(),
                0,
        ...
        // end extra debugging information
        let layout = Layout::from_size_align(size, utils::page_size()).unwrap();
        (&*MMAP).dealloc(base_ptr, layout);
    }
...
mod tests {
    ...
    fn layout_lookup() {
        ...
        test_and_free(4 << 20, |size, align| {
            alloc_assert_eq!((size, align), (4 << 20, utils::page_size()))
        });
    }
